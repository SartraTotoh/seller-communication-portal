...
const authStateFile=path.join(localStateDir,'workspace-auth-bridge-r44.json');
const reportDir=path.join(root,'ACCESS_RECOVERY_REPORT_R4_4');
...
function verifyFirebase(cli){
  ...
  let stdout = String(r.stdout||'').trim();
  let j;
  try{
    const jsonStart = stdout.indexOf('{');
    const jsonEnd = stdout.lastIndexOf('}');
    if(jsonStart>=0 && jsonEnd>=jsonStart){
      j=JSON.parse(stdout.slice(jsonStart, jsonEnd+1));
    } else {
      j=JSON.parse(stdout);
    }
  }catch{
    if(stdout.includes('seller-communication-portal')||stdout.includes(projectId))return true;
    return false;
  }
  const rows=Array.isArray(j?.result)?j.result:Array.isArray(j)?j:[];
  const hit=rows.find(p=>String(p.projectId||'')===projectId);
  if(!hit){
    if(stdout.includes(projectId))return true;
    return false;
  }
...
async function verifyHosting(marker){
  const urls=['https://seller-communication-portal.web.app/release-v4.9.3-r4.4.json','https://seller-communication-portal.firebaseapp.com/release-v4.9.3-r4.4.json'];
...
out('===============================================================');
out(' Seller Communication Portal v4.9.3R4.4 - WORKSPACE LOGIN BRIDGE');
...
let bridgeRun=runNode(path.join(toolsDir,'ensure-auth-bridge-r44.mjs'),['--bridge-dir',path.join(root,'auth-bridge'),'--state',authStateFile]);
...
const upd=runNode(path.join(toolsDir,'update-apps-script-deployment.mjs'),['--script',backend.scriptId,'--deployment',backend.deploymentId,'--description','Seller Communication Portal v4.9.3R4.4 Workspace Login Bridge','--required-access','DOMAIN','--required-execute-as','USER_DEPLOYING','--required-release',release,'--backend-dir',tempBackend]);
...
fs.writeFileSync(path.join(firebaseRoot,'public','release-v4.9.3-r4.4.json'),JSON.stringify({portal:'seller-communication-portal',release:'4.9.3R4.4',marker,auth:'SIGNED_WORKSPACE_BRIDGE',dataReset:false},null,2),'utf8');
...
writeJson(backendStateFile,{scriptId:backend.scriptId,deploymentId:newDeploymentId,projectId,updatedAt:new Date().toISOString(),mode:'ACCESS_R4_4_TOP_LEVEL_HANDOFF'});
fs.mkdirSync(reportDir,{recursive:true});
const report={release:'4.9.3R4.4',status:'COMPLETE',projectId,backendScriptId:backend.scriptId,backendDeploymentId:newDeploymentId,backendAccess:'DOMAIN',backendExecuteAs:'USER_DEPLOYING',authBridgeScriptId:bridgeDiag.scriptId,authBridgeDeploymentId:bridgeDiag.deploymentId,authBridgeAccess:'DOMAIN',authBridgeExecuteAs:'USER_ACCESSING',authScope:'userinfo.email only',googleSheetsReset:false,trackingLinksReset:false,shortLinkRoutesReset:false,setupRun:false,completedAt:new Date().toISOString()};
writeJson(path.join(reportDir,'WORKSPACE-LOGIN-R4.4.json'),report);
...
out('===============================================================','green');
out('\nOpening Workspace verification page... Click the orange Continue button once.','cyan');
