...
pass('Portal version 4.8.2',/data-portal-version="4\.8\.2"/.test(front));
...
pass('Backend closeout release 4.8.2',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.8\.2['"]/.test(closeout));
pass('Backend release 4.8.2',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.8\.2['"]/.test(tracker));
pass('Live Sync setup release 4.8.2',/LIVE_SYNC_SETUP_RELEASE', '4\.8\.2'/.test(live) && /release:'4\.8\.2'/.test(live));
...
pass('Deploy verifies v4.8.2 before complete',/Production serves Portal v4\.8\.2/.test(deploy) && /could not be verified[\s\S]*not marked complete/i.test(deploy));
...
pass('Safety gate checks Closeout.gs current release 4.8.2',closeoutGateLine.includes('4\\.8\\.2') && !closeoutGateLine.includes('4\\.8\\.0') && !closeoutGateLine.includes('4\\.8\\.1'),closeoutGateLine.trim());
pass('Safety gate requires Closeout.gs file',/Join-Path \$BackendDir 'Closeout\.gs'/.test(deploy));
pass('Deployer validator is current release',/validate-v4\.8\.2\.mjs/.test(deploy));
...
pass('One-click requires source-attested release before setup',/sourceAttested/.test(deploy) && /attestedRelease/.test(deploy) && /backend release 4\.8\.2/.test(deploy));
...
// v4.8.2 Triple-Source + Campaign 360 contracts
...
console.log(`Seller Communication Portal v4.8.2 validation: ${checks.length-failures.length}/${checks.length} PASS`);
