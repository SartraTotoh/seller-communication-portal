...
pass('Portal version 4.9.0',/data-portal-version="4\.9\.0"/.test(front));
...
pass('Backend closeout release 4.9.0',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.9\.0['"]/.test(closeout));
pass('Backend release 4.9.0',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.9\.0['"]/.test(tracker));
...
pass('Single deploy release constant is 4.9.0',/\$Release\s*=\s*['\"]4\.9\.0['\"]/.test(deploy));
...
const updaterSelfTest=spawnSync(process.execPath,[path.join(root,'tools/update-apps-script-deployment.mjs'),'--self-test','--backend-dir',path.join(root,'backend'),'--required-release','4.9.0'],{encoding:'utf8'});
...
// v4.9.0 Triple-Source + Campaign 360 contracts
...
console.log(`Seller Communication Portal v4.9.0 validation: ${checks.length-failures.length}/${checks.length} PASS`);
