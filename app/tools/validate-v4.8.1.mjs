...
pass('Portal version 4.8.1',/data-portal-version="4\.8\.1"/.test(front));
...
pass('Backend closeout release 4.8.1',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.8\.1['"]/.test(closeout));
pass('Backend release 4.8.1',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.8\.1['"]/.test(tracker));
pass('Live Sync setup release 4.8.1',/LIVE_SYNC_SETUP_RELEASE', '4\.8\.1'/.test(live) && /release:'4\.8\.1'/.test(live));
...
pass('Deploy verifies v4.8.1 before complete',/Production serves Portal v4\.8\.1/.test(deploy) && /could not be verified[\s\S]*not marked complete/i.test(deploy));
...
pass('One-click requires source-attested release before setup',/sourceAttested/.test(deploy) && /attestedRelease/.test(deploy) && /backend release 4\.8\.1/.test(deploy));
...
// v4.8.1 Triple-Source + Campaign 360 contracts
...
console.log(`Seller Communication Portal v4.8.1 validation: ${checks.length-failures.length}/${checks.length} PASS`);
