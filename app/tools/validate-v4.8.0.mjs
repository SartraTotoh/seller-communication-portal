...
pass('Portal version 4.8.0',/data-portal-version="4\.8\.0"/.test(front));
...
pass('Backend closeout release 4.8.0',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.8\.0['"]/.test(closeout));
pass('Backend release 4.8.0',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.8\.0['"]/.test(tracker));
pass('Live Sync setup release 4.8.0',/LIVE_SYNC_SETUP_RELEASE', '4\.8\.0'/.test(live) && /release:'4\.8\.0'/.test(live));
...
pass('Deploy verifies v4.8.0 before complete',/Production serves Portal v4\.8\.0/.test(deploy) && /could not be verified[\s\S]*not marked complete/i.test(deploy));
...
pass('One-click requires source-attested release before setup',/sourceAttested/.test(deploy) && /attestedRelease/.test(deploy) && /backend release 4\.8\.0/.test(deploy));
...
pass('v4.8.0 People self-heal is non-destructive for existing rows',/function ensurePeopleSeedV480ForSetup_/.test(seed) && /mode:'SKIP_EXISTING'/.test(seed) && /SETUP_SELF_HEAL_MISSING/.test(seed));
pass('v4.8.0 only repairs current bundled ADMIN before first READY',/SETUP_SELF_HEAL_ADMIN/.test(seed) && /adminSeed/.test(seed) && /liveSyncSetupReadyV480_/.test(seed));
pass('v4.8.0 refuses bootstrap overwrite after prior READY',/Automatic bootstrap repair is disabled to protect current role\/status decisions/.test(seed));
...
pass('Portal page/settings structure preserved',(front.match(/<section class="page[^>]*data-page-panel=/g)||[]).length===12 && (front.match(/data-settings-tab=/g)||[]).length===7);
...
console.log(`Seller Communication Portal v4.8.0 validation: ${checks.length-failures.length}/${checks.length} PASS`);
