...
pass('Portal version 4.8.5',/data-portal-version="4\.8\.5"/.test(front));
...
pass('Backend closeout release 4.8.5',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.8\.5['"]/.test(closeout));
pass('Backend release 4.8.5',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.8\.5['"]/.test(tracker));
...
pass('Single deploy release constant is 4.8.5',/\$Release\s*=\s*['\"]4\.8\.5['\"]/.test(deploy));
...
const updaterSelfTest=spawnSync(process.execPath,[path.join(root,'tools/update-apps-script-deployment.mjs'),'--self-test','--backend-dir',path.join(root,'backend'),'--required-release','4.8.5'],{encoding:'utf8'});
...
// v4.8.5 Triple-Source + Campaign 360 contracts
...
console.log(`Seller Communication Portal v4.8.5 validation: ${checks.length-failures.length}/${checks.length} PASS`);
