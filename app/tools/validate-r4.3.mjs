...
const recovery=text(path.join(tools,'r43-workspace-login-recovery.mjs'));
const bridgeHelper=text(path.join(tools,'ensure-auth-bridge.mjs'));
...
if(fail){console.error(`R4.3 validation failed: ${fail}`);process.exit(1);}console.log('R4.3 validation PASS');
