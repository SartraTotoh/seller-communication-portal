...
pass('Portal version 4.8.3',/data-portal-version="4\.8\.3"/.test(front));
...
pass('Backend closeout release 4.8.3',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.8\.3['"]/.test(closeout));
pass('Backend release 4.8.3',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.8\.3['"]/.test(tracker));
pass('Live Sync setup release 4.8.3',/LIVE_SYNC_SETUP_RELEASE', '4\.8\.3'/.test(live) && /release:'4\.8\.3'/.test(live));
...
pass('Single deploy release constant is 4.8.3',/\$Release\s*=\s*['\"]4\.8\.3['\"]/.test(deploy));
...
pass('No stale 4.8.1/4.8.2 in current release-critical runtime',!/4\.8\.[12]/.test(criticalRuntime));
...
// v4.8.3 Triple-Source + Campaign 360 contracts
...
console.log(`Seller Communication Portal v4.8.3 validation: ${checks.length-failures.length}/${checks.length} PASS`);
