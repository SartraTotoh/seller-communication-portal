...
pass('Portal version 4.7.5',/data-portal-version="4\.7\.5"/.test(front));
...
pass('Backend closeout release 4.7.5',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.7\.5['"]/.test(closeout));
pass('Backend release 4.7.5',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.7\.5['"]/.test(tracker));
...
pass('Deploy verifies v4.7.5 before complete',/Production serves Portal v4\.7\.5/.test(deploy) && /could not be verified[\s\S]*not marked complete/i.test(deploy));
...
console.log(`Seller Communication Portal v4.7.5 validation: ${checks.length-failures.length}/${checks.length} PASS`);
