...
pass('Portal version 4.7.9',/data-portal-version="4\.7\.9"/.test(front));
...
pass('Backend closeout release 4.7.9',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.7\.9['"]/.test(closeout));
pass('Backend release 4.7.9',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.7\.9['"]/.test(tracker));
pass('Live Sync setup release 4.7.9',/LIVE_SYNC_SETUP_RELEASE', '4\.7\.9'/.test(live) && /release:'4\.7\.9'/.test(live));
...
pass('Deploy verifies v4.7.9 before complete',/Production serves Portal v4\.7\.9/.test(deploy) && /could not be verified[\s\S]*not marked complete/i.test(deploy));
...
pass('Live Sync setup authenticates before writes',/function setupPortalLiveSync\(\)[\s\S]{0,260}?assertAdminUser_\(email\)/.test(live));
pass('Live Sync setup seeds People after admin gate',/assertAdminUser_\(email\)[\s\S]*ensurePeopleSeedV474IfNeeded_\(email\)/.test(live));
...
pass('Beginner Guidance stylesheet present',/id="beginner-guidance-v479"/.test(front));
...
console.log(`Seller Communication Portal v4.7.9 validation: ${checks.length-failures.length}/${checks.length} PASS`);
