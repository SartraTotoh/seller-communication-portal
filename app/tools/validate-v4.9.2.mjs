...
pass('Portal version 4.9.2',/data-portal-version="4\.9\.2"/.test(front));
...
pass('Backend closeout release 4.9.2',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.9\.2['"]/.test(closeout));
pass('Backend release 4.9.2',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.9\.2['"]/.test(tracker));
...
pass('Single deploy release constant is 4.9.2',/\$Release\s*=\s*['\"]4\.9\.2['\"]/.test(deploy));
...
const updaterSelfTest=spawnSync(process.execPath,[path.join(root,'tools/update-apps-script-deployment.mjs'),'--self-test','--backend-dir',path.join(root,'backend'),'--required-release','4.9.2'],{encoding:'utf8'});
...
// v4.9.2 Triple-Source + Campaign 360 contracts
...
console.log(`Seller Communication Portal v4.9.2 validation: ${checks.length-failures.length}/${checks.length} PASS`);
