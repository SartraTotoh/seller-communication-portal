...
try{
  ...
  const v=await request(`https://script.googleapis.com/v1/projects/${encodeURIComponent(scriptId)}/versions`,token,'POST',{description:'Seller Communication Portal v4.9.3R4.3 Workspace login bridge'});
  ...
  const updateBody={deploymentConfig:{scriptId,versionNumber,manifestFileName:'appsscript',description:'Seller Communication Portal v4.9.3R4.3 Workspace Auth Bridge'}};
  const createBody={versionNumber,manifestFileName:'appsscript',description:'Seller Communication Portal v4.9.3R4.3 Workspace Auth Bridge'};
  let deploy;
  if(deploymentId){
