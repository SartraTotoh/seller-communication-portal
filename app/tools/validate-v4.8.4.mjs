...
pass('Portal version 4.8.4',/data-portal-version="4\.8\.4"/.test(front));
...
pass('Backend closeout release 4.8.4',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.8\.4['"]/.test(closeout));
pass('Backend release 4.8.4',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.8\.4['"]/.test(tracker));
pass('Live Sync setup release 4.8.4',/LIVE_SYNC_SETUP_RELEASE', '4\.8\.4'/.test(live) && /release:'4\.8\.4'/.test(live));
...
pass('Single deploy release constant is 4.8.4',/\$Release\s*=\s*['\"]4\.8\.4['\"]/.test(deploy));
...
pass('Live Sync release safety gate uses centralized release',/\$livePattern[\s\S]*\$ReleaseRegex/.test(deploy) && /Live Sync setup release is not/.test(deploy));
...
pass('No stale 4.8.1/4.8.2/4.8.3 in current release-critical runtime',!/4\.8\.[123]/.test(criticalRuntime));
...
pass('Updater self-heals remote HEAD manifest before immutable version',/selfHealHeadPolicy/.test(updater) && /projects\/\$\{encodeURIComponent\(scriptId\)\}\/content/.test(updater) && /'PUT',\{files:patched\}/.test(updater));
pass('Remote manifest self-heal preserves all remote source files',/writableFiles\(files\)/.test(updater) && /patched\[pidx\]\.source=JSON\.stringify\(manifest/.test(updater));
pass('Remote manifest self-heal requires Google read-back',/REMOTE_MANIFEST_READBACK_FAILED/.test(updater) && /waitForHeadPolicy\(scriptId,token,manifestFileName/.test(updater));
...
pass('Updater verifies immutable source attestation',/verifyVersionAttestation/.test(updater) && /VERSION_SOURCE_ATTESTATION_FAILED/.test(updater) && /PEOPLE_SELF_HEAL_ORDER/.test(updater));
...
// v4.8.4 Triple-Source + Campaign 360 contracts
...
pass('Portal page/settings structure preserved',(front.match(/<section class="page[^>]*data-page-panel=/g)||[]).length===13 && (front.match(/data-settings-tab=/g)||[]).length===7);
...
console.log(`Seller Communication Portal v4.8.4 validation: ${checks.length-failures.length}/${checks.length} PASS`);
