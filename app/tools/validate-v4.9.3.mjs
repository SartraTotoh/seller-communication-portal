...
pass('Portal version 4.9.3',/data-portal-version="4\.9\.3"/.test(front));
...
pass('Backend closeout release 4.9.3',/PORTAL_CLOSEOUT_RELEASE\s*=\s*['"]4\.9\.3['"]/.test(closeout));
pass('Backend release 4.9.3',/PORTAL_BACKEND_RELEASE\s*=\s*['"]4\.9\.3['"]/.test(tracker));
...
pass('Single deploy release constant is 4.9.3',/\$Release\s*=\s*['\"]4\.9\.3['\"]/.test(deploy));
...
const updaterSelfTest=spawnSync(process.execPath,[path.join(root,'tools/update-apps-script-deployment.mjs'),'--self-test','--backend-dir',path.join(root,'backend'),'--required-release','4.9.3'],{encoding:'utf8'});
...
// v4.9.3 Triple-Source + Campaign 360 contracts
...
const writeMap=['submitRequest','updateRequest','saveReviewDecision','saveReviewComment','saveCycleRule','createSmartLink','getSmartLinkProviderStatus','configureTinyUrl','repairSmartLinks','createContent','analyzeContent','uploadArtwork','saveLookup','deleteLookup','saveBusinessConfig','saveScoringModelVersion','queueApiPublication','testApiConnection','approveApiPublication','sendApiPublication','saveTeamMember','massImportUsers'];
...
const routes=['request.create','request.update','review.decision','review.comment','cycle.save','smartlink.create','smartlink.status','smartlink.configure','smartlink.backfill','content.create','content.analyze','artwork.upload','lookup.save','lookup.delete','config.save','scoring.version','api.queue','api.status','api.approve','api.send','people.save','people.massImport','sync.now'];
...
pass('Smart Link schemas self-heal without reset',/ensureSheetColumns_\(ss,'Tracking_Links'/.test(closeout) && /ensureSheetColumns_\(ss,'Short_Link_Routes'/.test(closeout));
pass('Smart Link new create fails closed without real short URL',/if\(!shortUrl\)throw new Error\(tinyUrlUserMessage_/.test(api));
pass('TinyURL uses modern authenticated create API',/https:\/\/api\.tinyurl\.com\/create/.test(api) && /Authorization:'Bearer '\+token/.test(api));
pass('TinyURL transient retry present',/attempt<=3/.test(api) && /code===429\|\|code>=500/.test(api));
pass('Custom alias conflict auto-fallback present',/CREATED_AUTO_FALLBACK/.test(api) && /tinyUrlAliasConflict_/.test(api));
pass('Existing Smart Link repair is non-destructive',/DATA-SAFE repair/.test(api) && /writeFields_\(found,\{short_url:result\.shortUrl,status:nextStatus,updated_at:new Date\(\)\}\)/.test(api));
const repairBody=fnBody(api,'repairExistingSmartLinks_');
pass('Repair never clears or deletes rows',!!repairBody && !/clearContent|deleteRow|deleteRows|deleteSheet|clear\(/.test(repairBody));
pass('Repair does not rewrite destination or UTM fields',!!repairBody && !/writeFields_\([^\n]+(?:generated_url|original_url|utm_source|utm_medium|utm_campaign|utm_content|utm_term)/.test(repairBody));
pass('Short route insert is deduplicated',/Short_Link_Routes'\)\.find/.test(api) && /tracking_id/.test(api));
pass('Preview-only Smart Link fabrication removed',!/Preview only - TinyURL backend not connected/.test(front));
pass('Smart Link provider status UI present',/id="smartLinkProviderStatus"/.test(front) && /Connect TinyURL/.test(front) && /Repair existing links/.test(front));
pass('TinyURL token stays server-side',/PropertiesService\.getScriptProperties\(\)\.setProperty\('TINYURL_API_TOKEN'/.test(api) && !/localStorage\.setItem\([^\n]*TINYURL_API_TOKEN/.test(front));
...
console.log(`Seller Communication Portal v4.9.3 validation: ${checks.length-failures.length}/${checks.length} PASS`);
...
const consentTests=spawnSync(process.execPath,[path.join(root,'tools/test-consent.mjs')],{encoding:'utf8'});console.log(consentTests.stdout);if(consentTests.status!==0){console.error(consentTests.stderr);process.exitCode=1;}

const smartLinkTests=spawnSync(process.execPath,[path.join(root,'tools/test-smartlink-v4.9.3.mjs')],{encoding:'utf8'});console.log(smartLinkTests.stdout);if(smartLinkTests.status!==0){console.error(smartLinkTests.stderr);process.exitCode=1;}
